...
def load_and_validate(data_path: Path) -> pd.DataFrame:
      ...
      log.info(f"Delayed rows: {n_delayed:,} ({n_delayed / len(df):.1%} of total)")
      return df
...
def run_regression(data_path: Path, tune: bool = True) -> Path:
      ...
      X_train_full, ohe, feature_names = build_feature_matrix(train_full, fit_encoder=True)
      X_test_full, _, _ = build_feature_matrix(test_full, ohe=ohe, fit_encoder=False)
      ...
      y_train = train_full.loc[train_delayed_mask, TARGET_COL].reset_index(drop=True)
      X_train = X_train.reset_index(drop=True)

      X_test = X_test_full.loc[test_delayed_mask]
      y_test = test_full.loc[test_delayed_mask, TARGET_COL].reset_index(drop=True)
      ...
      all_metrics["lightgbm_untuned"] = compute_metrics(lgbm_init, X_test, y_test)
      log.info(
...
def two_stage_predict(
      ...
      if delayed_mask.any():
            ...
            X_reg, _, _ = build_feature_matrix(delayed_rows, ohe=reg_ohe, fit_encoder=False)
...
if __name__ == "__main__":
      parser = argparse.ArgumentParser(description="TTC Phase 1 Stage 2 — Delay Duration Regression")
      parser.add_argument(
            "--data",
            type=Path,

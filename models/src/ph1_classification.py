...
def run_classification(data_path: Path, tune: bool = True) -> Path:
    ...
    # ── 3. Feature encoding ───────────────────────────────────────────────
    X_train, ohe, feature_names = build_feature_matrix(
        train_df, fit_encoder=True)
    X_test,   _,    _ = build_feature_matrix(
        test_df, ohe=ohe, fit_encoder=False)
    ...
    stratified_model = DummyClassifier(
...
if __name__ == "__main__":
    ...
    artifact_dir = run_classification(
        data_path=args.data, tune=not args.no_tune)

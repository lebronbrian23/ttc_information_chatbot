...
def run_retrain(retrain_args: str = "") -> bool:
      ...
      try:
            ...
            if success:
                  ...
                  log.error(f"Retrain failed (exit code {result.returncode}) after {duration:.0f}s")
...
def run_scheduler() -> None:
      ...
      if next_run:
            log.info(f"Next scheduled retrain: {next_run.strftime('%Y-%m-%d %H:%M UTC')}")
      ...
      try:
            while True:
                  ...
                  if is_due(config, status.get("last_run_at")):
                        ...
                        if success:
                              status["runs_completed"] = status.get("runs_completed", 0) + 1
                        else:
...
def cli_status() -> None:
      ...
      if config["schedule"] == "weekly":
            print(f"   Day:                  {config.get('day_of_week', 'sunday').capitalize()}")
      print(f"   Time (UTC):       {config.get('hour', 2):02d}:{config.get('minute', 0):02d}")
      ...
      print(f"   Next run:          {next_run.strftime('%Y-%m-%d %H:%M UTC') if next_run else 'On demand only'}")
...
if __name__ == "__main__":
      ...
      parser = argparse.ArgumentParser(description="TTC Model Retraining Scheduler")

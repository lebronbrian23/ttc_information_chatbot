...
_PID_FILE = Path(__file__).resolve(
).parents[2] / "models" / "trained" / "ml_service.pid"
...
_LOOKUP_PATH = Path(os.getenv("TTC_LOOKUP_PATH")) if os.getenv(
    "TTC_LOOKUP_PATH") else None
...
app = FastAPI(title="TTC Delay Prediction API",
                    description=("Two-stage ML inference service for TTC subway delay prediction. "
...
# Write PID file — notifier.py reads this to find the process to signal
    ...
    try:
        get_predictor(clf_version=_CLF_VERSION,
...
class PredictRequest(BaseModel):
    ...
    threshold: float = Field(default=_DELAY_THRESHOLD,
                             ge=0.0, le=1.0,
...
async def scheduler_status() -> Dict[str, Any]:
    ...
    """
    status_path = Path(__file__).resolve(
    ).parents[2] / "models" / "trained" / "scheduler_status.json"
    config_path = Path(__file__).resolve(
    ).parents[2] / "models" / "trained" / "scheduler_config.json"
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Could not read scheduler status: {exc}")

    return {"schedule":            config.get("schedule", "unknown"),
...
async def predict(request: PredictRequest) -> Dict[str, Any]:
    ...
    try:
        predictor = get_predictor()
        result = predictor.predict(line=request.line,
...
async def predict_batch(request: BatchPredictRequest) -> List[Dict[str, Any]]:
    ...
    try:
        predictor = get_predictor()
        return predictor.predict_batch([r.model_dump(exclude={"threshold"}) for r in request.requests],
                                       threshold=request.threshold,
                                       )
    except Exception as exc:
        log.error(f"Batch prediction error: {exc}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Batch prediction failed: {exc}")

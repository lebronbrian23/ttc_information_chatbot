...
def load_subscribers() -> List[Dict[str, Any]]:
    ...
    subscribers = [s for s in data.get(
        "subscribers", []) if s.get("enabled", True)]
...
def notify_all(
    ...
    for subscriber in subscribers:
        results[subscriber["name"]] = send_webhook(
            subscriber, payload, dry_run=dry_run)
...
def restart_ml_service(dry_run: bool = False) -> bool:
    ...
    try:
        ...
        log.info(
            f"Sent SIGHUP to ML service (PID {pid}) — graceful reload initiated")
        return True
